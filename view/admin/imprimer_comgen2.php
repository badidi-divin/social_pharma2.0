<?php
    session_start();
    require_once '../../controller/select-com3.php';
        ...
        <a href="commande2.php">Retour>>>></a>
                ...
                <div class="panel panel-default">
                    ...
                    <div class="panel-heading">
                        LISTE DES COMMANDES DU <?= $_GET['de']." " ?> AU  <?= $_GET['a']." " ?><a onclick="window.print();">ici</a>
                    </div>
                                        ...
                                        <tfoot>
                                            <tr>
                                                <th style="font-size:20px">Total_PT:<?php
                                                    $nblmd=$pdo->prepare("SELECT SUM(pt) AS prix_total FROM commande WHERE dates>='$mot1' AND dates<='$mot2' ORDER BY dates");
                                                    $nblmd->execute();
                                                    $nblmd=$nblmd->fetch()['prix_total'];
                                                    echo $nblmd;
                                                ?><?= $userinfo['monaie'];?></th>
                                           <th style="font-size:20px">Total_REMISE:<?php
                                                    $nblmd=$pdo->prepare("SELECT SUM(red) AS prix_total FROM commande WHERE dates>='$mot1' AND dates<='$mot2' ORDER BY dates");
                                                    $nblmd->execute();
                                                    $nblmd=$nblmd->fetch()['prix_total'];
                                                    echo $nblmd;
                                                ?><?= $userinfo['monaie'];?></th>
