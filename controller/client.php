...
// ********************Affichage des clients****************************
   ...
   $mot2=isset($_GET['mot2'])?$_GET['mot2']:"";
	...
	if (isset($_GET['search'])) {
		$requete="SELECT * FROM client WHERE nom_complet LIKE '%$mot1%' AND date LIKE '%$mot2%'";			
	}else{
...
if (isset($_GET['dk'])) {
		...
		if (empty($errors)) {
		    ...
		    //Création de l'objet prepare et envoie de la requête
		    $ps=$pdo->prepare("UPDATE client SET nom_complet=?,sexe=?,telephone=?,email=?,adresse=? WHERE id=?");
		    //Pour bien recupere les données on crée un table de parametre
		    $params=array($nom_complet,$sexe,$telephone,$email,$adresse,$_GET['dk']);
		    //Execution de la requête par leur paramètre en ordre 
		    $ps->execute($params);
...
// *************Edition client**************************************
	...
	$requeteprint="SELECT * FROM client WHERE date LIKE '%$dates%'";	
